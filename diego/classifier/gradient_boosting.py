...
class GradientBoostingClassifier(
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        learning_rate = UniformFloatHyperparameter(
            name="learning_rate", lower=0.05, upper=0.5, default_value=0.1, log=True)
        max_iter = UniformIntegerHyperparameter(
            name="max_iter", lower=50, upper=500, default_value=100, log=True)
        min_samples_leaf = UniformIntegerHyperparameter(
            name="min_samples_leaf", lower=10, upper=200, default_value=20, log=True)
        max_depth = CategoricalHyperparameter(
            name="max_depth", choices=[3, 4, 5, 6, 7, 8], default_value=4)
        max_leaf_nodes = UniformIntegerHyperparameter(
            name="max_leaf_nodes", lower=8, upper=255, default_value=31, log=True)
        max_bins = Constant("max_bins", 255)
        l2_regularization = UniformFloatHyperparameter(
            name="l2_regularization", lower=1E-7, upper=1, default_value=1E-5, log=True)
        ...
        validation_fraction = CategoricalHyperparameter(
            name="validation_fraction", choices=[0.1, 0.15, 0.2], default_value=0.15)
